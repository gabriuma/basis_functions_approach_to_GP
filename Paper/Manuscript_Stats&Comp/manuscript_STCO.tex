...
There is a clear relation between the number of basis functions $m$ and the boundary factor $c$ with the length-scale $\ell$ of the approximated function. In this Section we discuss about the existing theoretical evidence that the minimum number of basis functions $m$ needed for an accurate approximation is linearly proportional to the the boundary factor $c$ and inversely proportional to the lengthscale $l$. In next Section \ref{subsec_empiric_relations} we conduct an empirical experiments to prove this expected linear proportionality relationship between $m$, $c$ and $l$ and compute the specific coefficients of proportionality for every kernel.
...
Figures \ref{fig6_lscale_vs_J_vs_c} and \ref{fig6_7_relationships} depict how these three factors interact and affect the accuracy of the HSGP approximation for a GP with square exponential covariance function and Mat\'ern ($\nu$=3/2) covariance function, respectively, and a single input dimension. More precisely, for a given GP model (with a squared exponential covariance function) with length-scale $\ell$ and given a boundary factor $c$, Figure \ref{fig6_lscale_vs_J_vs_c} shows the minimum number of basis functions $m$ required to obtain an accurate approximation in the sense of satisfying eq. (\ref{eq_diff_covs}). Similarly,  Figure \ref{fig6_7_relationships} shows the corresponding plot for the  Mat\'ern ($\nu$=3/2) covariance function. We considered an approximation to be a sufficiently accurate when the total variation difference between the approximate and exact covariance functions, $\varepsilon$ in eq. (\ref{eq_diff_covs}), is below 1$\%$ of the total area under the curve of the exact covariance function $k$ such that
%
\begin{equation*}
...
\item For a given $\ell$ there exist a minimum $c$ under which a accurate approximation is never going to be achieved regardless of $m$. This fact can be seen in Figures~\ref{fig6_lscale_vs_J_vs_c} and \ref{fig6_7_relationships} as the contour lines which represent $c$ have an end in function of $\ell$ (Valid $c$ are restricted in function of $\ell$). As $\ell$ increases, the minimum valid $c$ also increases.
\end{itemize}

\subsubsection{Numerical equations}

From the empirical observations the hypothesis of linear proportionality between $m$, $\frac{l}{S}$ (lenghscale $l$ relative to the half range of the input domain $S$) and $c$ has been proved. Following we show numerical functions governing these relationships for every kernel.
...
%  \label{fig7_lscale_vs_J_vs_c_Matern}
%\end{figure*}

%\begin{figure*}
%\centering
%\subfigure{\includegraphics[scale=0.38, trim = 0mm 0mm 5mm 0mm, clip]{fig6_7_relationships.pdf}}
%\hspace{3mm}
%\subfigure{\includegraphics[scale=0.38, trim = 0mm 0mm 5mm 0mm, clip]{fig7_lscale_vs_J_vs_c_zoomin_Matern.pdf}}
...
\begin{figure*}
\centering
\subfigure{\includegraphics[width=\textwidth, trim = 0mm 0mm 0mm 0mm, clip]{fig6_7_relationships.png}}
...
\subsubsection{User guide with the steps to perform diagnosis}

\paragraph*{Assumption of the diagnosis tool}: Under inaccurate HSGP approximation, the estimated lengthscale $\hat{l}$ is smaller than the exact GP lengthscale estimate $l$. Thus the diagnostic that determines is the approximation is sufficiently accurate is:
%
\begin{align}\label{eq_diagnostic}
&\hat{l} \pm 0.02 \geq l
\end{align}

\paragraph*{User-guide with the steps to perform diagnosis}:

\begin{enumerate}
\item[Step 1.] Make a first guess on the lengthscale $l_1$ of the function to be learned. We recommend a large lengthscale, around 1.
\item[] \hspace{-1cm} \textit{(Initial iteration, $i=1$)}
\item[Step 2.] Obtain the valid minimum boundary factor $c_1$ determined by $l_1$ by using (\ref{eq_c_vs_l_QE}).
\item[Step 3.] Obtain the valid mimimum number of basis functions $m_1$ determined by $l_1$ and $c_1$ by using (\ref{eq_m_l_QE}). Notice that $l_1$ can also be read as the minimum lengthscale that can be accurately fitted determined by $m_1$ and $c_1$ by using (\ref{eq_m_l_QE}).
\item[Step 4.] Fit the HSGP model and assess residuals: compute \textit{rmse*} (Root mean square error), $R^2$ (Coefficient of variation) and \textit{elpd} (Expected log predicitve density).
\item[Step 5.] Check the diagnostic of whether $\, \hat{l_1} \pm 0.02 \geq l_1$ (\ref{eq_diagnostic}).
	\begin{enumerate}
    \item If the diagnostic is TRUE, the HSGP model approximation must be close to be sufficiently accurate. Then user can continue with step 6.
       	\begin{enumerate}
    	\item[] \hspace{-1cm} \textit{(Next iterations, $i>1$)}
		\item[Step 6.] Set $\, m_i = m_{i-1} + 10$.
		\item[Step 7.] Obtain the valid minimum boundary factor $c_i$ by using (\ref{eq_c_vs_l_QE}), with $c(\hat{l}_{i-1})$.
		\item[Step 8.] Obtain the mimimum lengthscale $l_i$ that can be accurately fitted determined by $m_i$ and $c_i$ by using (\ref{eq_m_l_QE}).
		\item[Step 9.] Fit the HSGP model and
			\begin{enumerate}
        	\item[Step 9.1.] Check whether $\, \hat{l}_i \pm 0.02 \geq l_i$ (\ref{eq_diagnostic}),
        	\item[Step 9.2.] Check stability in $\hat{l}_i$, \textit{rmse}, $R^2$ and \textit{elpd} relative to previous iteration.
			\end{enumerate}
		\item[Step 10.] If the verifications in steps 9.1 and 9.2 are TRUE, the HSGP model approximation should be sufficiently accurate, and diagnosis ends here. Otherwise, repeat steps 6-10 and update parameters.
    	\end{enumerate}
     \item If the diagnostic is FALSE, the HSGP model approximation can not be sufficiently accurate. Then user has to move to step 11.
     	\begin{enumerate}
		\item[Step 11.] Set $\, l_i = \hat{l}_{i-1}$.
		\item[Step 12.] Repeat steps 2-5 and update parameters.
		\end{enumerate}
    \end{enumerate}
