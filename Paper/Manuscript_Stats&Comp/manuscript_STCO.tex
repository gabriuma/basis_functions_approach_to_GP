...
%             \emph{Present address:} of F. Author  %  if needed
           ...
           Paul-Christian Bürkner \at
              Department of Computer Science, University of Stuttgard, Germany \\
              \emph{Most of work was done while at Aalto University}
           \and
...
\begin{figure*}
\centering
\subfigure{\includegraphics[scale=0.38, trim = 0mm 0mm 5mm 0mm, clip]{fig7_lscale_vs_J_vs_c_Matern.pdf}}
\hspace{3mm}
...
%The diagnosis consists in comparing the estimated length-scale $\hat{\ell}$ for a given $m$ and $c$, with that minimum length-scale $\ell^*$ that can be accurately approximated, for those given $m$ and $c$, generated by the model of relationships in Figures \ref{fig6_lscale_vs_J_vs_c} and \ref{fig7_lscale_vs_J_vs_c_Matern}. Thus, if $\hat{\ell} < \ell^*$, $m$ needs to increased or $c$ decreased, and this can be iteratively until we get $\hat{\ell} \geq \ell^*$. 
Figure \ref{fig13_diagnosis} depicts the diagnosis applied for this case study. In the first iteration, we choose $m=20$ and $c=1.5$, which turns out to be an invalid combination of $m$ and $c$, as $c=1.5$ is too small for $m=20$ or $m=20$ is too small for $c=1.5$, as can be seen in the corresponding plot of Figure \ref{fig13_diagnosis}. In the second iteration, $m$ is increased up to $m=30$, the HSGP estimates a length-scale of $\hat{\ell}=0.12$, and the minimum length-scale that can be accurately approximated, for those $m$ and $c$, generated by the model of relationships in Figure \ref{fig7_lscale_vs_J_vs_c_Matern} is $\ell^*=0.19$. As $\hat{\ell} < \ell^*$, the approximation is diagnosed to be inaccurate (see Figure \ref{fig12_MSE_exI_inter}-left where the interpolation error of HSGP is higher than that of GP). In the third iteration, $m$ is increased up to $m=40$, the HSGP model estimates a length-scale of $\hat{\ell}=0.14$, and the model of relationships brings a minimum approachable length-scale of $\ell^*=0.15$. As $\hat{\ell}$ is still lower than $\ell^*$, the approximation is diagnosed to be inaccurate, but since $\hat{\ell}$ is quite close to $\ell^*$, the error is expected to be small (see errors in Figure \ref{fig12_MSE_exI_inter}-left). In the fourth iteration, $m$ is increased up to $m=60$, the estimated length-scale is $\hat{\ell}=0.14$ and the minimum approachable length-scale generated by the model of relationships is $\ell^*=0.11$, and as $\hat{\ell} > \ell^*$ then approximation is diagnosed to be accurate.
