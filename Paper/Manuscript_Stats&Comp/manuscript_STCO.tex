...
\item For a given $\ell$ there exist a minimum $c$ under which a accurate approximation is never going to be achieved regardless of $m$. This fact can be seen in Figures~\ref{fig6_7_relationships} and \ref{fig6_7_relationships}
% TODO FIX the figure reference!
as the contour lines which represent $c$ have an end in function of $\ell$ (Valid $c$ are restricted in function of $\ell$). As $\ell$ increases, the minimum valid $c$ also increases.
\end{itemize}
...
\begin{enumerate}
\item[Step A1.] Make an initial guess on the lengthscale $l_1$. If there is no useful information available, we recommend to start with a large lengthscale, about $l_1 \approx 1$. Set the iteration index to $i=1$.
