functions {
	//GP 1D with a quared exponential (SE) kernel
	vector gp_1D_SE(real[] x, real sdgp, real lscale, vector zgp) { 
		int n= size(x);
		matrix[n, n] cov;
	...
	//GP 1D with a Mattern (SE) kernel
	vector gp_1D_Mattern(real[] x, real sdgp, real lscale, vector zgp, int v) {
		int n= size(x);	
		matrix[n, n] cov;
		//Mattern 3/2
		if (v==3)
	...
	//Predicting GP 1D for SE kernel (Analytical prediction)
	vector gp_1D_SE_rng(real[] x2, vector f1, real[] x1, real gpscale, real lscale, real delta) {
	int N1 = rows(f1);
	int N2 = size(x2);
	vector[N2] f2;
	 {
  ...
  //Predicting GP 1D for Mattern kernel (Analytical prediction)
	vector gp_1D_Mattern_rng(real[] x2, vector f1, real[] x1, real gpscale, real lscale, real delta, real v) {
	int N1 = rows(f1);
	int N2 = size(x2);
	vector[N2] f2;
	matrix[N1, N1] K;
...
transformed parameters{
	vector[N_train] f;
	f = gp_1D_Mattern(x_pred[vv_train], gpscale, lscale, eta, param_v);
	// f = gp_1D_SE(x[vv_train], gpscale, lscale, eta);	
}
model{
	eta ~ normal(0, 1);
	lscale ~ gamma(1.2,0.2); //gamma(2.5, 10); //gamma(3.75, 25); //normal(0,1); //
	sigma ~ normal(0,1); //gamma(1, 1);
...
generated quantities{
	...
	f_pred = gp_1D_Mattern_rng(x_pred, f, x_pred[vv_train], gpscale, lscale, 1e-12, param_v);
	for(i in 1:N_pred){
		elpd_y[i] = normal_lpdf(y[i] | f_pred[i], sigma);
		elpd_f[i] = normal_lpdf(f_true[i] | f_pred[i], sigma);
	}
}
