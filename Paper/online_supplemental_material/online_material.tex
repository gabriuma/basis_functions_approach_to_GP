...
In the case of the first-order components, the normalized length-scale estimates $\left(\frac{2\cdot \hat{\ell}_{d,1}}{|x^d_{max}-x^d_{min}|}\right)$ are all bigger than the minimum approachable length-scale reported by Figure (6) % \ref{fig6_lscale_vs_J_vs_c} 
