...
Online supplemental material with more case studies illustrating the performance and applicability of the method can be found at \url{https://github.com/gabriuma/basis_functions_approach_to_GP} in the subfolder \texttt{Paper/online\_appendices}.
...
%
\begin{align*}
k_{\infty}(r)&=\alpha \exp\bigg(-\frac{1}{2} \frac{r^2}{\ell^2}\bigg),  \\
k_{\frac{3}{2}}(r)&=\alpha\bigg(1+\frac{\sqrt{3}r}{\ell}\bigg) \exp\bigg(-\frac{\sqrt{3}r}{\ell}\bigg). 
\end{align*}
...
%
\begin{align*}
k_{\infty}(|\bm{x}-\bm{x}'|_{L2})&=\alpha \exp\bigg(-\frac{1}{2} \sum_{i=1}^{D}\frac{(x_i-x_i')^2}{\ell_i^2}\bigg),\\
k_{\frac{3}{2}}(|\bm{x}-\bm{x}'|_{L2})&=\alpha \Bigg(1+\sqrt{\sum_{i=1}^{D}\frac{3(x_i-x_i')^2}{\ell_i^2}}\Bigg)\exp\Bigg(- \sqrt{\sum_{i=1}^{D}\frac{3(x_i-x_i')^2}{\ell_i^2}}\Bigg). \\
\end{align*}
...
%
\begin{align}
S_{\infty}(\omega)&= \alpha \sqrt{2\pi}^D  \ell^D  \exp\left(-0.5 \ell^2 \omega^2\right), \label{eq_specdens_inf}  \\
...
%
\begin{align*}
S_{\infty}(\omega)&= \alpha \sqrt{2\pi}^{3}  \prod_{i=1}^{3} \ell_i  \mathrm{exp}\left(-\frac{1}{2} \sum_{i=1}^{3} \ell_i^2 s_i^2 \right),   \\
...
Figure \ref{ch5_fig11_time_exI} shows computational times, in seconds per iteration (iteration of the HMC sampling method), as a function of the number of basis functions $m$, for the HSGP model, and knots, for the spline model. The HSGP model is on average roughly 400 times faster than the regular GP, in this particular application.

The Stan model code for the exact GP, the approximate GP and the spline models of this case study can be found online at \url{https://github.com/gabriuma/Doctoral_thesis/tree/master/Case-study-I_1D-Simulated-data}.
%
...
The Stan model code for the approximate GP model of this case study can be found at \url{https://github.com/gabriuma/Doctoral_thesis/tree/master/Case-study-II_Birthday-data}.
...
The Stan model code for the exact GP and the approximate GP models of this case study can be found at \url{https://github.com/gabriuma/Doctoral_thesis/tree/master/Case-study-VII_Leukemia-data}.
%
...
The main drawback of the approach is that its computational complexity scales exponentially with the number of input dimensions. Hence, in practice, input dimensionalities larger than 3 start to be quite computationally demanding. In these cases, the proposed approxiamate GP method may still be used for low-dimensional components in an additive modeling scheme but without modeling very high dimensional interactions.
...
k(\bm{\tau})= \alpha \exp\Big(-\frac{2\text{sin}^2\left(\omega_0\frac{\bm{\tau}}{2}\right)}{\ell^2}\Big),
\end{eqnarray}
...
k(\tau)= \alpha \sum_{j=0}^{J} \tilde{q}_j^2 \text{cos}(j\omega_0 \tau),
\end{eqnarray}
