...
model{
	beta ~ normal(0,1);
	rho ~ gamma(4,1);  #normal(2,3);
	alpha ~ inv_gamma(2,5); #normal(3,3);
...
generated quantities{
	vector[N1+N2] f;  #+Npred
	vector[N1+N2] y_predict;  #+Npred
	vector[N1+N2] log_y_predict;
	vector[N1+N2] f_invlogit; #+Npred	
	
	f[ind2]= PHI[ind2,] * SPD_beta;
	...
	for(i in 1:(N1+N2)){
		log_y_predict[i] = binomial_logit_lpmf(Y[i] | 1, f[i]);
	}
