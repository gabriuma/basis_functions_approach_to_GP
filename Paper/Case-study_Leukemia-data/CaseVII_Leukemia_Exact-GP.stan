...
data {
	...
	vector[D] X[Nobs+Ncens];			#matrix of inputs
	vector[D] Xgrid[Ngrid];				#matrix of inputs
...
parameters {
	# real<lower=0> rho[D];  			#multiple lengthscales
	real<lower=0> rho[2]; 					#one lengthscale
	
	# real<lower=0> alpha[D];  			#additive model
	real<lower=0> alpha[2];
	
	real<lower=0> sigma;
	matrix[Nobs+Ncens,2] eta;
