functions {
  //Eigenvalue function
	...
	//Square root vector of spectral densities
  ...
  matrix PHI_3D(int N, int M1, int M2, int M3, real L1, real L2, real L3, vector x1, vector x2, vector x3) {
    int k;
    matrix[N,M1*M2*M3] PHI;
    matrix[N,M1] PHI_1 = sin(diag_post_multiply(rep_matrix(pi()/(2*L1) * (x1+L1), M1), linspaced_vector(M1, 1, M1)))/sqrt(L1);
    matrix[N,M2] PHI_2 = sin(diag_post_multiply(rep_matrix(pi()/(2*L2) * (x2+L2), M2), linspaced_vector(M2, 1, M2)))/sqrt(L2);
    matrix[N,M3] PHI_3 = sin(diag_post_multiply(rep_matrix(pi()/(2*L3) * (x3+L3), M3), linspaced_vector(M3, 1, M3)))/sqrt(L3);
    for(j in 1:M1){
      k = M2*M3*(j-1);
      PHI[,(k+1):(k+M3)] = rep_matrix(PHI_1[,j], M3) .* rep_matrix(PHI_2[,1], M3) .* PHI_3;
      for(i in 2:M2){
        PHI[,(k+1):(k+M3*i)] = append_col(PHI[,(k+1):(k+M3*(i-1))], rep_matrix(PHI_1[,j], M3) .* rep_matrix(PHI_2[,i], M3) .* PHI_3);
      }
    }
    return PHI;
  }
...
transformed data {
	matrix[N_sample,M_nD] PHI;
  PHI = PHI_3D(N_sample, M[1], M[2], M[3], L[1], L[2], L[3], x_pred[vv_sample,1], x_pred[vv_sample,2], x_pred[vv_sample,3]);
}
...
transformed parameters{
 ...
 {
	vector[M_nD] diagSPD;
	diagSPD = sqrt_diagSPD_nD(gpscale, lscale, L, indices, D);
