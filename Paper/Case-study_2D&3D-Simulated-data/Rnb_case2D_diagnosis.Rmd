---
title: '2D simulated case study: Diagnosis of the HSGP approximation'
...
This notebook implements the HSGP model fit and diagnosis on the 2D dataset in case study 5.3 '*Simulated data for 2D and 3D functions*' in [Riutort-Mayol, G., Bürkner, P. C., Andersen, M. R., Solin, A., & Vehtari, A. (2020; updated in 2022). Practical Hilbert space approximate Bayesian Gaussian processes for probabilistic programming. arXiv preprint arXiv:2004.11408.](https://arxiv.org/pdf/2004.11408.pdf)
...
A synthetic dataset $\boldsymbol{y}$ with a true data generating function $f(\boldsymbol{x})$, with $\boldsymbol{x}_i \in {\rm I\!R}^2$, where $f(\boldsymbol{x})$ is a Gaussian process (GP) with a squared exponential kernel. Gaussian errors $\boldsymbol{\epsilon}$ are added to $f(\boldsymbol{x})$ to form the final synthetic noisy dataset $\boldsymbol{y}$.
...
where $k(\boldsymbol{x}, \boldsymbol{x}', \theta)$ is a squared exponential kernel as a function of input values $\boldsymbol{x}$ and parameters $\theta=(\alpha,\ell)$, where $\alpha$ is the marginal variance of the GP and $\ell$ the lengthscale of the kernel.

## Draw the true data generative function $f(x)$ from a GP 

### Input predictive space

Matrix of 2D-predictive input points `x_pred`, length of `x_pred` (`N_pred`)

```{r eval=TRUE}
x_pred <- pred_grid(coords= c(-1,1), y.coords= c(-1,1), by= 0.04)  
N_pred <- dim(x_pred)[1]
N_pred
```
...
simuGP_mod <- cmdstanr::cmdstan_model(stan_file = "stancode_SIMU_2D.stan")
...
brk <- seq(-4.00,max(f_true), length.out= 12)

filled.contour(xx, xx, matrix(f_true,length(xx),length(xx)), levels = brk, col = brewer.pal(11,"BrBG")[11:1], plot.axes= {axis(1, tick=FALSE, line=-1.1, font=5, mgp= c(2.5, 0.1, 0), las=1, cex.axis=1.5); title(xlab =expression(x[1]), mgp= c(0.8, 1, 0), cex.lab=2, las=1, family="serif"); axis(2, tick=FALSE, line=0.4, font=5, mgp= c(2.5, 0.0, 0), las=1, cex.axis=1.5); abline(h= seq(-1,1,0.5), col=grey(0.7)); lines(c(-1,-1,-0.5,-0.5,0,0,0.5,0.5,1,1), c(-1.03,1.03,-1.03,1.03,-1.03,1.03,-1.03,1.03,-1.03,1.03), col=grey(0.7), type="h")}, asp= 1, frame.plot=FALSE, xlim=c(-1.03,1.03), ylim=c(-1.03,1.03))
text(-0.3,0.9,"(a)", font=2, cex=1.5);
text( par("usr")[1] - 0.30, mean(c(-1,1)), labels = expression(x[2]), srt = 0, xpd = TRUE, cex=2, font=1, family="serif")
```
...
GP_mod <- cmdstanr::cmdstan_model(stan_file = "stancode_GP_marginalized_2D.stan")
...
param = c("lscale","gpscale[1]","noise")
...
gp12_f_pred <- summary(fit_GP, pars = c("f_pred"), probs = c(0.025, 0.5, 0.975), digits_summary = 4)$summary[,c(1,4,6)]
gp12_elpd <- summary(fit_GP, pars= c("elpd"), probs = c(0.025, 0.5, 0.975), digits_summary= 4)$summary[,c(1,4,6)]
gp12_time <- sum(get_elapsed_time(fit_GP)[2])/(fit_GP@sim$iter - fit_GP@sim$warmup)
gp12_mse <- sqrt(sum((f_true - gp12_f_pred[,1])^2)/N_pred)


col1 <- brewer.pal(11,"BrBG")[11:1] 
# brk <- seq(min(gp12_f_pred[,1]),max(gp12_f_pred[,1]), length.out= 12)

filled.contour(xx, xx, matrix(gp12_f_pred[,1],length(xx),length(xx)), levels = brk, col = brewer.pal(11,"BrBG")[11:1], plot.axes= {axis(1, tick=FALSE, line=-1.1, font=5, mgp= c(2.5, 0.1, 0), las=1, cex.axis=1.5); title(xlab =expression(x[1]), mgp= c(0.8, 1, 0), cex.lab=2, las=1, family="serif"); axis(2, tick=FALSE, line=0.4, font=5, mgp= c(2.5, 0.0, 0), las=1, cex.axis=1.5); points(x_pred[vv_sample,], cex=1, lwd=1, pch=1);  abline(h= seq(-1,1,0.5), col=grey(0.7)); lines(c(-1,-1,-0.5,-0.5,0,0,0.5,0.5,1,1), c(-1.03,1.03,-1.03,1.03,-1.03,1.03,-1.03,1.03,-1.03,1.03), col=grey(0.7), type="h")}, asp= 1, frame.plot=FALSE, xlim=c(-1.03,1.03), ylim=c(-1.03,1.03)) #; grid(col=grey(0.90), lty=1)
text(-0.3,0.9,"(b)   GP", font=2, cex=1.5);
text( par("usr")[1] - 0.30, mean(c(-1,1)), labels = expression(x[2]), srt = 0, xpd = TRUE, cex=2, font=1, family="serif")
```
...
```{r eval=TRUE, include=TRUE, echo=FALSE}
error <- f_true - gp12_f_pred[,1]

col3 <- c(colorRampPalette(c("blue", "white"), bias=10)(6), colorRampPalette(c("white", "red"), bias=10)(6)[2:6])#brewer.pal(11,"RdBu")[1:11]
brk_error <- seq(min(error),max(error), length.out= 12)

filled.contour(xx, xx, matrix(error,length(xx),length(xx)), levels= brk_error, col= col3, plot.axes= {axis(1, tick=FALSE, line=-1.1, font=5, mgp= c(2.5, 0.1, 0), las=1, cex.axis=1.5); title(xlab =expression(x[1]), mgp= c(0.8, 1, 0), cex.lab=2, las=1, family="serif"); axis(2, tick=FALSE, line=0.4, font=5, mgp= c(2.5, 0.0, 0), las=1, cex.axis=1.5); points(x_pred[vv_sample,], cex=1, lwd=1, pch=1);  abline(h= seq(-1,1,0.5), col=grey(0.6)); lines(c(-1,-1,-0.5,-0.5,0,0,0.5,0.5,1,1), c(-1.03,1.03,-1.03,1.03,-1.03,1.03,-1.03,1.03,-1.03,1.03), col=grey(0.6), type="h")}, asp= 1, frame.plot=FALSE, xlim=c(-1.03,1.03), ylim=c(-1.03,1.03))
text(-0.3,1.12,"(a)", font=2, cex=1.5);
text( par("usr")[1] - 0.30, mean(c(-1,1)), labels = expression(x[2]), srt = 0, xpd = TRUE, cex=2, font=1, family="serif")
```
...
# Diagnostic function over the lengthscale of the HSGP model

The lengthscale diagnosis function is defined as:
...
if(i==1){
  l1[i] <- 0.5
  c1[i] <- c_vs_l_QE(l=l1[i], S)
  m1[i] <- m_QE(c=c1[i], l=l1[i], S=S)
  
  l2[i] <- 1
  c2[i] <- c_vs_l_QE(l=l2[i], S)
  m2[i] <- m_QE(c=c2[i], l=l2[i], S=S)
}
```
...
```{r eval=TRUE, results=FALSE, message=FALSE, warn=FALSE}
hsgp_mod <- cmdstanr::cmdstan_model(stan_file = "stancode_HSGP_2D.stan")
...
```{r eval=TRUE}
param = c("lscale[1]","lscale[2]","gpscale","noise")
...
```{r eval=TRUE, include=TRUE}
f <- summary(fit[[i]], pars = c("f"), probs = c(0.025, 0.5, 0.975), digits_summary = 4)$summary[,c(1,4,6)]

# rmse against data
residual <- standata[[i]]$y_pred[vv_sample] - f[,1]
rmse[i] <- round(sqrt(mean(residual^2)), 2)

# rmse against data
residual_gp <- gp12_f_pred[vv_sample] - f[,1]
...
diagnosis[[i]] <- data.frame(iter= c(i, i),
                              ...
                              rsme_gp = c(NA, rmse_gp[i]),
                              rsme = c(NA, rmse[i]),
                              R2 = c(NA, eR[i]),
                              elpd = c(NA, elpd[i]))
names(diagnosis[[i]]) <- c("iter", "Dim", "l", "c", "m", "l_hat", "l_hat > l", "rmse_gp", "rmse", "R2", "elpd") 
...
if(diagnostic(l1[i-1], l_hat1[i-1]) & i==2){
  m1[i] <- m1[i-1] + 2
  c1[i] <- c_vs_l_QE(l=l_hat1[i-1], S=S)
  l1[i] <- l_QE(c=c1[i], m=m1[i], S=S)

  m2[i] <- m2[i-1] + 2
  c2[i] <- c_vs_l_QE(l=l_hat2[i-1], S=S)
  l2[i] <- l_QE(c=c2[i], m=m2[i], S=S)
}
```
...
if(!diagnostic(l1[i-1], l_hat1[i-1])){
  l1[i] <- l_hat1[i-1]
  c1[i] <- c_vs_l_QE(l=l1[i], S=S)
  m1[i] <- m_QE(c=c1[i], l=l1[i], S=S)
  
  l2[i] <- l_hat2[i-1]
  c2[i] <- c_vs_l_QE(l=l2[i], S=S)
  m2[i] <- m_QE(c=c2[i], l=l2[i], S=S)
}
```
...
Lengthscale estimate $\hat{\ell_i}$ in the HSGP approximation

```{r eval=TRUE, include=TRUE}
l_hat1[i] <- round(summary(fit[[i]], pars = "lscale[1]", probs = c(0.05, 0.95))$summary[,1], 3)
l_hat2[i] <- round(summary(fit[[i]], pars = "lscale[2]", probs = c(0.05, 0.95))$summary[,1], 3)
```

Check the diagnostic of whether $\, \hat{\ell_i} + 0.01 \geq \ell_i$ \@ref(eq:diagnostic)
...
If the diagnostic $\hat{\ell}_{i-1} + 0.01 \geq \ell_{i-1}$ \@ref(eq:diagnostic) is TRUE and iteration $i>2$ and diagnostic $\hat{\ell}_{i-2} \geq \ell_{i-2}$ \@ref(eq:diagnostic) is FALSE:
...
Check the diagnostic of whether $\, \hat{\ell_i} + 0.01 \geq \ell_i$ 
